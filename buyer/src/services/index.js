...
const getReqWithoutToken = async (path) => {
  ...
  const API_ENDPOINT = `http://192.168.1.102:4000/api${path}`
  return await axios.get(API_ENDPOINT, AXIOS_CONFIG);
}
...
const postAllowCookie = async(path, payload) => {
  const AXIOS_CONFIG = {
    headers: {
      "Access-Control-Allow-Origin": "http://192.168.1.102:4000"
    },
...
const postSendCookie = async(path) => {
  const AXIOS_CONFIG = {
    headers: {
      "Access-Control-Allow-Origin": "http://192.168.1.102:4000",
    },
  ...
  const API_ENDPOINT = `${baseUrl}${path}`;
  return await axios.post(API_ENDPOINT, AXIOS_CONFIG);
}
